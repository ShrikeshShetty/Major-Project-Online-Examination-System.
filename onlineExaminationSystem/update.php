...
if(isset($_SESSION['key'])){
    if(@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'sunny7785068889') {
        ...
        $time = $_POST['time'];
        $tag = $_POST['tag'];
        $id = uniqid(); // Generate a unique id for eid
  
        // Include eid in the insert query
        $q3 = mysqli_query($con, "INSERT INTO quiz (eid, title, sahi, wrong, total, time, date, tag) VALUES ('$id', '$name', '$sahi', '$wrong', '$total', '$time', NOW(), '$tag')") or die('Error: ' . mysqli_error($con));
